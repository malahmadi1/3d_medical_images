...
parser.add_argument('--root_path', type=str, default='C:/dataset/LASC2018', help='Name of Experiment') 
parser.add_argument('--root_pathlist', type=str, default='D:/deeplearning_Sanaz/SSL-contrastive/dataset/LA', help='Name of Experiment')               # todo change dataset path
...
parser.add_argument('--uncertainty', type=bool, default = True, help='use uncertainty map')
...
if __name__ == "__main__":
    ...
    def create_model(name ='vnet'):
        # Network definition
        if name == 'vnet':
            net = VNet(n_channels=1, n_classes=num_classes, normalization='batchnorm', has_dropout=True, use_uncertainty=args.uncertainty)
            model = net.cuda()
        if name == 'resnet34':
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #print(f'>>>>>>>>>>>>>>>>>>>> {r_outputs_soft.shape}')
            
            r_loss_seg_dice = losses.dice_loss(r_outputs_soft[:labeled_bs, 1, :, :, :], r_label[:labeled_bs] == 1)
            #print('v_outputs_soft',r_outputs_soft[1,1,:10,:10])
            uncertaincy_map = torch.where(r_outputs_soft< 0.7, torch.tensor(1.0), torch.tensor(0.0))
            uncertaincy_map = torch.sum(uncertaincy_map, dim = 1, keepdim=True)
            #print('uncertaincy_map',uncertaincy_map[1,1,:10,:10])
            #print(f'>>>>>>>>>>>>>>>>>>>> {r_outputs_soft.shape}')
            #print('uncertaincy_map',uncertaincy_map.shape)
            ####VNET
            v_outputs, v_rep = model_vnet(v_input,uncertaincy_map)
            ## calculate the supervised loss
            v_loss_seg = F.cross_entropy(v_outputs[:labeled_bs], v_label[:labeled_bs])
            v_outputs_soft = F.softmax(v_outputs, dim=1)

            #v_outputs_soft = v_outputs_soft()
