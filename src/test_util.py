...
def test_all_case(vnet,resnet, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, dataset_pan = False):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        if dataset_pan:
            id = image_path.split('/')[-1][:-3]
        else:
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                if vnet is not None:
                    y1,_ = vnet(test_patch)
                    y  = F.softmax(y1, dim=1)
                    y = y.cpu().data.numpy()
                    y = y[0,:,:,:,:]
                if resnet is not None:
                    y2, _ = resnet(test_patch)
